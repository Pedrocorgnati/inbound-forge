...
export default function KnowledgeError({ error, reset }: ErrorProps) {
  ...
  useEffect(() => {
    captureException(error, { digest: error.digest, route: pathname, boundary: 'knowledge-error' })
  }, [error, pathname])

  return (
    <div className="flex flex-col items-center justify-center py-24 text-center space-y-4">
      <p className="text-6xl font-bold text-danger/20">500</p>
      <h2 className="text-xl font-semibold text-foreground">Erro ao carregar Base de Conhecimento</h2>
      <p className="text-sm text-muted-foreground max-w-sm">
        Não foi possível carregar seus casos, dores ou padrões. Tente novamente.
      </p>
      {error.digest && (
