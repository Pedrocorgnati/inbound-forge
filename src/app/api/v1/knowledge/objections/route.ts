...
import { CreateObjectionSchema } from '@/schemas/knowledge.schema'
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.objection.findMany({ orderBy: { createdAt: 'desc' }, skip: (page - 1) * limit, take: limit }),
...
// POST /api/v1/knowledge/objections
export async function POST(request: NextRequest) {
  ...
  try {
    const entry = await prisma.objection.create({ data: parsed.data })
    return ok(entry, 201)
  } catch {
    return internalError()
  }
