...
// GET /api/v1/analytics/themes?period=&page=&limit=&sortBy=realConversionRate|priorityScore|leadsCount
// MS13-B001: sortBy aceita realConversionRate (taxa real, default), priorityScore (composto)
// e leadsCount. "conversionScore" mantido como alias legado de realConversionRate.
...
export async function GET(request: NextRequest) {
  ...
  const sortByParam = searchParams.get('sortBy') ?? 'realConversionRate'
  const sortDirParam = searchParams.get('sortDir') ?? 'desc'

  const validSortBy = ['realConversionRate', 'priorityScore', 'leadsCount', 'conversionScore']
  ...
  try {
    const result = await getThemeRanking(
      user!.id,
      sortByParam as 'realConversionRate' | 'priorityScore' | 'leadsCount' | 'conversionScore',
      page,
      limit,
      period,
