...
export function ContentPageClient({ locale }: ContentPageClientProps) {
...
function ContentListCard({ item, locale }: { item: ContentListItem; locale: string }) {
  ...
  return (
    ...
    >
      ...
      {item.updatedAt && (
        <p className="mt-1 text-xs text-muted-foreground">
          Atualizado: {fmt.date(item.updatedAt)}
        </p>
      )}

      <div className="mt-auto border-t border-border pt-3 mt-4 flex flex-col gap-2">
        ...
        {hasContent && item.status === 'DRAFT' && (
          <ContentDeleteButton
            piece={{ id: item.id, status: item.status, baseTitle: item.themeTitle }}
          />
        )}
      </div>
    </Card>
